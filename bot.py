...
from config import EXCHANGE_CONFIG, TRADING_CONFIG, STRATEGY_CONFIG, NOTIFICATION_CONFIG
...
class TradingBot:
    ...
    async def initialize(self):
        ...
        logger.info(f"✅ Bot initialized: {TRADING_CONFIG.symbol} on {EXCHANGE_CONFIG.name}")
    ...
    async def _execute_trade(self, signal: Dict, market_data: pd.DataFrame):
        ...
        if TRADING_CONFIG.paper_trading:
            # Paper trade
            if action == 'buy':
                position_size = min(TRADING_CONFIG.max_position_size, self.paper_balance * 0.1)
                ...
                position = {
                    ...
                    'take_profit': price * (1 + TRADING_CONFIG.take_profit_percent / 100),
    ...
    async def _close_position(self, position: Dict, exit_price: float, reason: str):
        ...
        emoji = "✅" if pnl > 0 else "❌"
        logger.info(f"{emoji} PAPER CLOSE ({reason}): PnL ${pnl:.2f} | Balance: ${self.paper_balance:.2f}")
        self.performance.record_trade(position, exit_price, pnl)
